...
struct GitSearchView: View {
    ...
    var body: some View {
        ...
        .toast(isPresenting: $downloadSuccess, alert: {
            AlertToast(displayMode: .alert, type: .regular,title: "다운로드 완료",subTitle: downloadDirect)
        })
        .toast(isPresenting: $downloadError,duration: 30, tapToDismiss: true, alert: {
            AlertToast(displayMode:.alert, type: .regular, title: "다운로드 실패",subTitle: downloadErrorCode)
        })
    }
